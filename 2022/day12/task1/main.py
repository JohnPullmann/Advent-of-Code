...
def hill_climbing(inp: list = [str]) -> int:
    """Climb a hill and return fewest steps required to move from your current position to destination"""
    class board:
        board = []
        layers = {}
        active_layer = 0
        ...
        def find_accessible_neighbours(self):
            ...
            for d in dirs:
                ...
                if 0 <= n_cell_pos[0] < len(board.board[0]) and 0 <= n_cell_pos[1] < len(board.board):
                    ...
                    if n_cell.cost == None and n_cell.height <= self.height+1:
